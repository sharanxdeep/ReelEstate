...
const CreateListing = () => {
  ...
  const submitListing = async (status) => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:3000/api/listings", {
        ...
        credentials: "include",
        body: JSON.stringify({ ...formData, chapters: uploadedChapters, status }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Failed to upload listing");
      navigate(status === "draft" ? "/profile" : "/feed");
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#001D39]">
      ...
      <div
        className="max-w-2xl mx-auto px-5 md:px-8 pb-32"
        ...
        <form onSubmit={(e) => e.preventDefault()} className="space-y-10">
            ...
            {formData.chapters.map((chapter, index) => (
              ...
              >
                ...
                <label className="flex items-center justify-center gap-2 w-full py-3 rounded-full border border-dashed border-white/20 text-white/60 text-[13px] cursor-pointer hover:border-white/40 transition-colors">
                  <Upload size={14} />
                  {chapter.videoFile ? chapter.videoFile.name : "Upload video"}
                  <input
                    type="file"
                    accept="video/*"
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              className="flex-1 py-3.5 rounded-4xl border border-white/20 text-white font-medium hover:bg-white/5 transition-colors disabled:opacity-50"
            >
              Save as draft
            </button>
            <button
