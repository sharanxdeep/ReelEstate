...
const deleteListing = async(req,res)=>{
    try{
        ...
        if (!listing) return res.status(404).json({message:"Listing not found"})
        if(listing.seller.toString()!==req.user._id.toString()) return res.status(403).json({message:"You are not authorized to delete this listing"})
...
module.exports = { createListing, getAllListings, getListing, deleteListing, updateListing}